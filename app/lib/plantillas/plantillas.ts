...
/**
 * Sustitución de llaves DOBLES con contrato duro (B6.2, 18-08): cada caller
 * declara SUS valores (§16 — {{importe}} no significa lo mismo en cobros que
 * en la cadencia), y NINGUNA llave sobrevive en un texto que se dé por bueno —
 * ni una doble sin dato, ni una SIMPLE de una plantilla mal escrita («Hola
 * {Ana}» ya casi le llega a un paciente). Si `sinResolver` no está vacío, el
 * texto NO se envía: se descarta y se cuenta.
 */
