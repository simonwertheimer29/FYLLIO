...
| ~~Medir el clasificador~~ **DESBLOQUEADO (12 ago)** | **Hay saldo** — sonda a la API: 200. El eval vuelve a correr (`npm run qa:evals`). Lo que sigue pendiente es **medir**: la regla de los dos intentos está implementada y sin número desde el 6 de agosto. El eval no corre: su sonda aborta con código 2, que es lo correcto —«no pude comprobar» no es «falla»— pero deja sin medir el último cambio. **Último número fiable: 98 % (42/43)**, de ANTES de la regla de los dos intentos, que está **implementada y sin medir**. Y hay **sospecha de que ese 98 % estuviera inflado**: la red vieja de «Sin clasificar → quiebra» cazaba por accidente disparadores de fase 2 (tono negativo, pide persona), y en las dos corridas con crédito antes de agotarse (89 % y 87 %) fallaban justo esos | Nadie — solo correr el eval |
