...
| **Enviar las 11 plantillas a Meta** | **Escritas el 7 ago** ([`PLANTILLAS-WHATSAPP.md`](PLANTILLAS-WHATSAPP.md)), sin enviar. Exige cuenta de Meta Business con nombre legal y NIF → **depende del alta fiscal**. Tenerlas escritas **NO adelanta plazo**: el reloj de Meta no ha empezado a correr. Y una de las once (`seguimiento_sigue_vigente`) está bloqueada además por dato — necesita el plazo de validez, que no existe ([MEJORAS 89](MEJORAS-PENDIENTES.md)) | Simon: revisar los textos + alta fiscal |
