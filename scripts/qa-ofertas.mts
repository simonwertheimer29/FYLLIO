...
console.log("══ textos (plantillas aprobadas)");
{
  ...
  ok(oferta.includes("Dinos cuál te va y queda reservada para ti.") && !/Si ninguna/.test(oferta), "sin la frase que da permiso a no elegir");
  ...
  ok(des.startsWith("Samuel, ¿cuál de estas dices?") && /1\) jue 24/.test(des) && /2\) vie 25/.test(des), "la desambiguación solo repite lo que ya salió");
