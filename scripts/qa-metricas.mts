...
async function limpiar() {
  await runWithClienteDb("DEMO", async (trx) => {
    ...
    await sql`delete from historial_acciones where presupuesto_id = 'qa-met-pres'`.execute(trx);
    await sql`delete from presupuestos where id = 'qa-met-pres'`.execute(trx);
...
async function main() {
  await runWithCliente("DEMO", async () => {
    ...
    await runWithClienteDb("DEMO", async (trx) => {
      ...
      // Un presupuesto PERDIDO ese día cuyo historial va SIN clinica_id (la forma
      // en que se escribió hasta el 9-sep, MEJORAS 217): la sede sale por el presupuesto.
      await sql`insert into presupuestos (id, cliente, paciente_id, clinica_id, estado, importe, fecha) values ('qa-met-pres', 'DEMO', 'qa-met-pac', ${CLINICA}, 'PERDIDO', 900, '2019-12-01'::date)`.execute(trx);
      await sql`insert into historial_acciones (cliente, presupuesto_id, tipo, descripcion, metadata, fecha) values ('DEMO', 'qa-met-pres', 'cambio_estado', 'QA', '{"estadoNuevo":"PERDIDO"}', '2020-01-15T13:00:00+01:00'::timestamptz)`.execute(trx);
    ...
    check(red.presupuestos_presentados_n?.valor === 0 && red.aceptados_eur?.valor === 0 && red.perdidos_n?.valor === 1, `presentados/aceptados = 0, perdidos = 1 por el historial (${red.perdidos_n?.valor})`);
    ...
    check(cli.perdidos_n?.valor === 1, `perdidos por clínica = 1 vía la clínica del PRESUPUESTO, aunque el historial no lleve sede (217) (${cli.perdidos_n?.valor})`);
    ...
    check(otra.entrantes?.valor === 0 && otra.pagos_eur?.valor === 0 && otra.leads_citados?.valor === 0 && otra.envios_tal_cual?.n === 0 && otra.perdidos_n?.valor === 0, "otra clínica no ve nada (tampoco los envíos medidos ni el perdido)");
