/**
 * scripts/setup-waba-config-table.ts
 * Crea la tabla Configuracion_WABA en Airtable.
 * Solo datos operativos (telemetría + flag por clínica).
 * Credenciales (Phone_Number_ID, Access_Token, etc.) viven en env vars de Vercel.
 * Idempotente: omite lo que ya existe.
 *
 * Uso:
 *   npx tsx scripts/setup-waba-config-table.ts
 */
...
const WABA_CONFIG_FIELDS = [
  { name: "Clinica", type: "singleLineText" },
  { name: "Activo", type: "checkbox", options: { icon: "check", color: "greenBright" } },
  { name: "Webhook_URL", type: "singleLineText" },
  { name: "Ultimo_mensaje_enviado", type: "dateTime", options: DATETIME_OPTIONS },
  { name: "Ultimo_mensaje_recibido", type: "dateTime", options: DATETIME_OPTIONS },
...
async function main() {
  ...
  await ensureTable(tables, "Configuracion_WABA", WABA_CONFIG_FIELDS);

  console.log("\n🎉 Setup de Configuracion_WABA completado.");
  console.log("   Recordatorio: las credenciales (Phone_Number_ID, Access_Token, etc.)");
  console.log("   van en env vars de Vercel, NO en esta tabla.");
}
